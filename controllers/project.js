...
const getPosts = async (req, res) => {
  const posts = await Post.find({ author: "Oluwaferanmi" });
...
const getPost = async (req, res) => {
  ...
  const post = await Post.findOne({ user: "Oluwaferanmi", _id: postId });
...
const updatePost = async (req, res) => {
  const post = await Post.findOneAndUpdate(
    { author: "Oluwaferanmi", _id: req.params.id },
    req.body,
